...
In previous file, we have identified 19 schools that show up in actual enrolment
counts but to which no pupil is ever assigned. Basically, it looks as if ALL
pupils are in these schools due to placing requests. That is a bit strange-

So, here I just take the imputation file and check what is going on.
...
cd "$rawdata04/checks"
merge m:1 seedcode using schoollist_unmatched2018
...
*br seedcode lacode pupilid postcode ratio 
/*
sum ratio, detail
hist ratio, width(1)
*/
...
merge 1:m postcode seedcode using pupillevel_composite`year'nodups
drop _merge 
