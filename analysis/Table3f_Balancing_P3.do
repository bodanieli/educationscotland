...
outreg2 using Table3f_Balancing_Planner`noplanner'_`subsample'.xls, replace title()
...
#delimit; 
cd "$finaloutput09/CFE"; 
outreg2 using Table3f_Balancing_Planner`noplanner'_`subsample'.xls, append title() ctitle("`var'") 
...
cd "$rawdata04/cfe" 
merge 1:1 pupilid wavetwo using p1_lagged_cfe
keep if _merge==3 
drop _merge
...
#delimit; 
cd "$finaloutput09/CFE"; 
outreg2 using Table3f_Balancing_Planner`noplanner'_`subsample'.xls, append title()
addtext(School FE, "No" , Planner-Calc Only, "`noplanner'", Stage, "`subsample'", Outcome, "`var'")
dec(3)
label nocons
