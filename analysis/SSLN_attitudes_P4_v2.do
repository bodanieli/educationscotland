...
areg `y' bottomcomp topcomp  $spec2 i.SSLNwave, absorb(seedcode) cluster(schoolyear) 
...
#delimit;
outreg2 using P4attitudes.xls, append  ctitle("OLS") 
...
/*OLS: */
areg `y' bottomcomp topcomp classsize $spec3 i.SSLNwave, absorb(seedcode) cluster(schoolyear) 
...
#delimit;
ivreg2hdfe, 
de(`y') endog(bottomcomp topcomp ) iv(comphighincentive_act complowincentive_act) ex($spec2) id1(seedcode) id2(SSLNwave)cluster(schoolyear)
; 
...
#delimit;
cd "$finaloutput09\extraMPOoutput";
outreg2 using P4attitudes.xls, append ctitle("2SLS") 
addtext(No. of  Schools, `schools', Covariates, "Yes", Class-Size Instrumented, "No" , School FE, "Yes" , Planner-Calc Only, "Yes", Stage, "`subsample'",  Outcome, "`:var label `y''")
adds(F-Stat, `fstat') 
dec(3)
label nocons
...
#delimit;
ivreg2hdfe, 
de(`y') endog(bottomcomp topcomp ) iv(comphighincentive_act complowincentive_act) ex(classsize $spec3) id1(seedcode) id2(SSLNwave)cluster(schoolyear)
; 
...
#delimit;
ivreg2hdfe, 
de(`y') endog(bottomcomp topcomp ) iv(comphighincentive_act complowincentive_act) ex(classhat $spec3) id1(seedcode) id2(SSLNwave)cluster(schoolyear)
; 
...
/*NOW OLDER PEERS
/*OLS Without Class Size: */
areg `y' olderpeers youngerpeers $spec2 i.SSLNwave, absorb(seedcode) cluster(schoolyear) 
...
/*OLS: */
areg `y' olderpeers youngerpeers classsize $spec3 i.SSLNwave, absorb(seedcode) cluster(schoolyear) 
...
#delimit;
ivreg2hdfe, 
de(`y') endog(olderpeers youngerpeers) iv(comphighincentive_act complowincentive_act) ex( $spec2) id1(seedcode) id2(SSLNwave)cluster(schoolyear)
; 
...
#delimit;
ivreg2hdfe, 
de(`y') endog(olderpeers youngerpeers) iv(comphighincentive_act complowincentive_act) ex(classsize $spec3) id1(seedcode) id2(SSLNwave)cluster(schoolyear)
; 
...
#delimit;
ivreg2hdfe, 
de(`y') endog(olderpeers youngerpeers) iv(comphighincentive_act complowincentive_act) ex(classhat $spec3) id1(seedcode) id2(SSLNwave)cluster(schoolyear)
; 
