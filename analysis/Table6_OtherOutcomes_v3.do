...
In particular, we explore effect of peers on other CFE outcomes,
such as writing, reading and listening attainment.  
...
cd "$finaloutput09\CFE\MainResults\otherCFEoutcomes" 
...
#delimit;
outreg2 using P1OtherCFEOutcomes.xls, append  ctitle("OLS") 
...
#delimit;
cd "$finaloutput09\CFE\MainResults\otherCFEoutcomes"; 
outreg2 using P1OtherCFEOutcomes.xls, append ctitle("2SLS") 
addtext(No. of  Schools, `schools', Covariates, "Yes", Class-Size Instrumented, "No" , School FE, "Yes" , Planner-Calc Only, "Yes", Stage, "`subsample'",  Outcome, "`:var label `y''")
adds(F-Stat, `fstat') 
dec(3)
label nocons
...
#delimit;
ivreg2hdfe, 
de(`y') endog(bottomcomp) iv(complowincentive_act) ex(classhat $spec2) id1(seedcode) id2(wave)cluster(schoolyear)
; 
