...
collapse (mean) $covariates101 $school_covariates101
$outcomes101 classsize rule_adj cohort_size_adj
bottomcomp olderpeers cohort_size_adj_sq 
(max)  complowincentive_act schoolyear
, by(stage seedcode wave)
...
label variable classsize "Stage-Level Average Class Size"

local subsample "P1"
keep if stage ==1

label variable bottomcomp "Share of Pupils in Bottom Composite"
label variable olderpeers "Stage-Level Average Older Peers"


label variable numeracy_atlevel "Numeracy - Performing at level (rate)"
label variable literacy_atlevel "Literacy - Performing at level (rate)"
...
#delimit;
outreg2 using P1MainResults_StageLevel.xls, append  ctitle("OLS") 
...
#delimit;
ivreg2hdfe, 
de(literacy_atlevel) endog(bottomcomp) iv(complowincentive_act) ex( $spec2) id1(seedcode) id2(wave) /*cluster(schoolyear)*/
; 
...
#delimit;
cd "$finaloutput09\CFE\appendix"; 
outreg2 using P1MainResults_StageLevel.xls, append ctitle("2SLS") 
addtext(No. of  Schools, `schools', Covariates, "Yes", Class-Size Instrumented, "No" , School FE, "Yes" , Planner-Calc Only, "Yes", Stage, "`subsample'",  Outcome, "`:var label `y''")
adds(F-Stat, `fstat') 
dec(3)
label nocons
...
#delimit;
ivreg2hdfe, 
de(literacy_atlevel) endog(bottomcomp) iv(complowincentive_act) ex(classsize $spec3) id1(seedcode) id2(wave) /*cluster(schoolyear)*/
; 
