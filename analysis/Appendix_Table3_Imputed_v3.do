...
#delimit;
outreg2 using P1ResultsImputedInstrument.xls, append  ctitle("OLS") 
...
#delimit;
ivreg2hdfe, 
de(`y') endog(bottomcomp) iv(complowincentive_imp) ex($spec2) id1(seedcode) id2(wave)cluster(schoolyear)
; 
...
#delimit;
cd "$finaloutput09\CFE\appendix"; 
outreg2 using P1ResultsImputedInstrument.xls, append ctitle("2SLS") 
addtext(No. of  Schools, `schools', Covariates, "Yes", Class-Size Instrumented, "No" , School FE, "Yes" , Planner-Calc Only, "Yes", Stage, "`subsample'",  Outcome, "`:var label `y''")
adds(F-Stat, `fstat') 
dec(3)
label nocons
...
#delimit;
ivreg2hdfe, 
de(`y') endog(bottomcomp) iv(complowincentive_imp) ex(classsize $spec3) id1(seedcode) id2(wave)cluster(schoolyear)
; 
...
#delimit;
ivreg2hdfe, 
de(`y') endog(bottomcomp) iv(complowincentive_imp) ex(classhat $spec3) id1(seedcode) id2(wave)cluster(schoolyear)
; 
...
#delimit;
ivreg2hdfe, 
de(`y') endog(olderpeers) iv(complowincentive_imp) ex($spec2) id1(seedcode) id2(wave)cluster(schoolyear)
; 
...
#delimit;
ivreg2hdfe, 
de(`y') endog(olderpeers) iv(complowincentive_imp) ex(classsize $spec3) id1(seedcode) id2(wave)cluster(schoolyear)
; 
...
#delimit;
ivreg2hdfe, 
de(`y') endog(olderpeers) iv(complowincentive_imp) ex(classhat $spec3) id1(seedcode) id2(wave)cluster(schoolyear)
; 
